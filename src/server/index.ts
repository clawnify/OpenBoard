...
app.openapi(updateBoard, async (c) => {
  ...
  await run(
    ...
    [body.name ?? existing.name, body.viewport_json ?? existing.viewport_json, id]
  );
...
app.openapi(createElement, async (c) => {
  ...
  await run(
    `INSERT INTO elements (board_id, type, x, y, width, height, rotation, z_index, props_json)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [boardId, body.type, body.x ?? 0, body.y ?? 0, body.width ?? 200, body.height ?? 200, body.rotation ?? 0, zIndex, body.props_json ?? "{}"]
  );
...
app.openapi(updateElement, async (c) => {
  ...
  await run(
    ...
    [body.x ?? existing.x, body.y ?? existing.y, body.width ?? existing.width, body.height ?? existing.height, body.rotation ?? existing.rotation, body.z_index ?? existing.z_index, body.props_json ?? existing.props_json, id]
  );
...
app.openapi(batchUpdateElements, async (c) => {
  ...
  for (const u of body.updates) {
    ...
    await run(
      `UPDATE elements SET x = ?, y = ?, width = ?, height = ?, rotation = ?, z_index = ?, props_json = ?, updated_at = datetime('now') WHERE id = ?`,
      [u.x ?? existing.x, u.y ?? existing.y, u.width ?? existing.width, u.height ?? existing.height, u.rotation ?? existing.rotation, u.z_index ?? existing.z_index, u.props_json ?? existing.props_json, u.id]
    );
  }
  return c.json({ ok: true }, 200);
});
